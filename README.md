...
`data/aliases.tsv` に**愛称**（表記が正式名と異なるニックネーム）・**表記揺れ**
（同じ表記に対して通用している別の読み）・**姓だけ／名だけ**（正式名の一部を切り出したもの）・
**短縮名からフルネームへの変換**（本人が名乗る短い呼び名で正式名を変換する）を収録しています。
...
SKK はロイスが使っていますが、 Microsoft IME・ATOK での実機確認は募集中です（Issue でお知らせください）。

### ヴ音（「ゔ」）を含む読みについて

`data/entries.tsv`・`data/aliases.tsv` の読みは「ゔ」（U+3094・結合済み1文字）で統一していますが、
**`skk.txt` だけは見出し語生成時に「う゛」（う U+3046 + 濁点 U+309B・2文字）へ変換します**。
SKK-JISYO.L（skk-dev/dict）の実例（`"ぐるーう゛かん /グルーヴ感/"` `"しう゛ぁ /湿婆/"`）で
確認したとおり、SKK 辞書は歴史的にヴ音を「う゛」の2文字で見出しに持ち、SKKIME 等で `vu` と
ローマ字入力した際もこの2文字が生成されるためです（`#2`）。

他の3形式（MS-IME・Google 日本語入力/Mozc・ATOK）はユーザー辞書の読みとして「ゔ」をそのまま
出力しています。各 IME が「ゔ」を読みとして受理できるかは**実機で未確認です**
（確認できた方は Issue でお知らせください）。
